const e={};function t(e){let t=document.createElement("div");t.innerHTML=`
  ...
  `;let t=this.shadow.querySelector(".resultado-titulo");t.textContent=this.getAttribute("quienGano");let i=this.shadow.querySelector(".li-score-vos");i.textContent=i.textContent+this.getAttribute("numVos");let n=this.shadow.querySelector(".li-score-maquina");n.textContent=n.textContent+this.getAttribute("numMaquina"),this.shadow.appendChild(e)}}customElements.define("result-escore-el",e)}(),e.subscribe(()=>{let t=e.getState(),i=JSON.stringify(t.history);localStorage.setItem("history",i)}),localStorage.getItem("history")){let t={currentGame:{computerPlay:"",myPlay:""},history:JSON.parse(localStorage.getItem("history")||"hola")};e.setState(t)}let t=document.querySelector(".root");!function(e){function t(e){let t=o()?n+e:e;history.pushState({},"",t),i(t)}function i(i){console.log("El handleRoute recibi\xf3 una nueva ruta ",i);let l=o()?i.replace(n,""):i;for(let i of a)if(i.path.test(l)){let n=i.component({goTo:t});for(;e.firstChild;)e.removeChild(e.firstChild);e.appendChild(n)}}i(location.pathname),window.onpopstate=function(e){i(location.pathname)}}(t)}();
